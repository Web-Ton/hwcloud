...
#   TUI_SMOKE_BIN=./hwcloud scripts/tui-smoke.sh
#
...
WORKDIR="$(mktemp -d)"
BIN="${TUI_SMOKE_BIN:-$WORKDIR/hwcloud-smoke}"
FAILED=""
...
BIN="${TUI_SMOKE_BIN:-$(ls "$WORKDIR"/hwcloud-smoke 2>/dev/null || echo "${TUI_SMOKE_BIN:?SKIP_BUILD=1 requires TUI_SMOKE_BIN}")}"
