...
type liveChild struct {
	...
	toolFilter func() []hwcloud.Tool // nil = use deps.Tools as-is
	mu         sync.Mutex
...
func (r *childRegistry) startAsync(child *liveChild, session hwcloud.Session, task, description string) error {
...
func (r *childRegistry) spawn(parentDeps Deps, cfg *agent.Agent, toolFilter func() []hwcloud.Tool) *liveChild {
	r.mu.Lock()
	r.counter++
...
type memSessionStore struct {
	mu         sync.Mutex
	msgs       []hwcloud.Message                     // append-only, oldest first
	summarizer hwcloud.Summarizer                    // nil = no compaction
	compressed map[string]*hwcloud.CompressedContext // keyed by sessionID
}
...
func newMemSessionStore(summarizer hwcloud.Summarizer) *memSessionStore {
	return &memSessionStore{
		compressed: make(map[string]*hwcloud.CompressedContext),
...
func (m *memSessionStore) Append(_ context.Context, _ string, msg hwcloud.Message) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *memSessionStore) Recent(_ context.Context, _ string, n int, offset int) ([]hwcloud.Message, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if n <= 0 || offset < 0 {
		return nil, nil
	...
	out := make([]hwcloud.Message, end-start)
	copy(out, m.msgs[start:end])
	return out, nil
}

func (m *memSessionStore) RecentAfter(_ context.Context, _ string, throughIndex, n int) ([]hwcloud.Message, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if throughIndex < 0 {
		throughIndex = 0
	...
	out := make([]hwcloud.Message, len(rest))
	copy(out, rest)
	return out, nil
}
...
func (m *memSessionStore) Compact(ctx context.Context, sessionID string, throughIndex int, messages []hwcloud.Message) error {
	if m.summarizer == nil {
...
func (m *memSessionStore) Compressed(_ context.Context, sessionID string) (*hwcloud.CompressedContext, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
