...
func (t *listTool) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{
		...
		Parameters: hwcloud.SchemaOf[listParams](),
	}
}

func (t *listTool) Execute(_ context.Context, _ json.RawMessage) *hwcloud.ToolResult {
	agents := t.reg.List()
	if len(agents) == 0 {
		return &hwcloud.ToolResult{Content: "No live sub-agents in this session."}
	}
	var b []byte
	...
	return &hwcloud.ToolResult{Content: fmt.Sprintf("Live sub-agents (%d):\n%s%s", len(agents), string(b), hint)}
}
