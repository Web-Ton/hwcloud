...
import (
	...
	"github.com/Cloud-Developer-Department/hwcloud/cmd/cli/tui/components"
	"github.com/Cloud-Developer-Department/hwcloud/cmd/cli/tui/theme"
	"github.com/Cloud-Developer-Department/hwcloud/cmd/cli/tui/views/chat"
...
func startACPInProcess(ctx context.Context, p *tea.Program, cfg config.Config, ver string) {
	...
	client := openacp.NewClient("hwcloud-tui", ver)
	sess := client.ConnectIO(ctx, clientW, clientR)

	if _, err := sess.Initialize(ctx, openacp.InitializeRequest{
		ProtocolVersion: 1,
		ClientInfo:      &openacp.Implementation{Name: "hwcloud-tui", Version: ver},
	}); err != nil {
