...
func riskCall() hwcloud.ToolCall {
	return hwcloud.ToolCall{
		ID:   "c2",
		Type: "function",
		Function: hwcloud.ToolCallFunction{
...
func TestApproverAutoModeAllowsRisk(t *testing.T) {
	...
	d, err := a.Ask(context.Background(), riskCall(), hwcloud.FunctionDefinition{Name: "shell"}, hwcloud.Session{})
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if d.Action != governance.Allow {
...
func TestApproverSemiAutoAllowsSafe(t *testing.T) {
	...
	d, err := a.Ask(context.Background(), sampleCall(), hwcloud.FunctionDefinition{Name: "read_file"}, hwcloud.Session{})
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if d.Action != governance.Allow {
