...
func (t *sendTool) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{
		...
		Parameters: hwcloud.SchemaOf[SendParams](),
	}
}

func (t *sendTool) Execute(ctx context.Context, args json.RawMessage) *hwcloud.ToolResult {
	params, err := hwcloud.ParseArgs[SendParams](args)
	if err != nil {
		return hwcloud.ErrorResult(fmt.Errorf("sub_agent_send: %w", err), false, "")
	}
	if params.AgentID == "" {
		return hwcloud.ErrorResult(fmt.Errorf("sub_agent_send: agent_id is required"), false, "")
	}
	if params.Message == "" {
		return hwcloud.ErrorResult(fmt.Errorf("sub_agent_send: message is required"), false, "")
	}
	...
	if err != nil {
		return &hwcloud.ToolResult{
			Error: &hwcloud.ToolError{Message: fmt.Sprintf("sub_agent_send: %v", err)},
		}
	}
