...
func (m *Model) ListModels(ctx context.Context) ([]hwcloud.AvailableModel, error) {
	page, err := m.client.Models.List(ctx)
	if err != nil {
		return nil, fmt.Errorf("list models: %w", err)
	}
	out := make([]hwcloud.AvailableModel, 0, len(page.Data))
	for _, mo := range page.Data {
		am := hwcloud.AvailableModel{
			ID:             mo.ID,
			OwnedBy:        mo.OwnedBy,
...
func applyExtensionFields(am *hwcloud.AvailableModel, rawJSON string) {
...
var _ hwcloud.ModelLister = (*Model)(nil)
