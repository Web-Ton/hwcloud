...
func (t *checkDriftTool) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{
		Name: "check_drift",
		...
		Parameters: hwcloud.SchemaOf[CheckDriftParams](),
	}
}

func (t *checkDriftTool) Execute(ctx context.Context, args json.RawMessage) *hwcloud.ToolResult {
	params, err := hwcloud.ParseArgs[CheckDriftParams](args)
	if err != nil {
		return hwcloud.ErrorResult(fmt.Errorf("check_drift: %w", err), false, "")
	}
	if !validDeploymentID(params.DeploymentID) {
		return hwcloud.ErrorResult(fmt.Errorf("check_drift: invalid deployment_id %q", params.DeploymentID), false, "")
	}
