...
// same operations as the `hwcloud settings` CLI subcommand (set/get/
...
func (t *settingsTool) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{
		...
		Parameters: hwcloud.SchemaOf[settingsParams](),
	}
}
...
func (t *settingsTool) Execute(ctx context.Context, args json.RawMessage) *hwcloud.ToolResult {
	p, err := hwcloud.ParseArgs[settingsParams](args)
	if err != nil {
		return hwcloud.ErrorResult(fmt.Errorf("settings: %w", err), false, "")
	}
	action := strings.TrimSpace(p.Action)
	switch action {
	case "set":
		if strings.TrimSpace(p.Key) == "" {
			return hwcloud.ErrorResult(fmt.Errorf("settings set: key is required"), false, "")
		}
		if err := config.SetSetting(p.Key, p.Value); err != nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings set: %w", err), true, "")
		}
		...
		return &hwcloud.ToolResult{Content: msg}
	case "get":
		if strings.TrimSpace(p.Key) == "" {
			return hwcloud.ErrorResult(fmt.Errorf("settings get: key is required"), false, "")
		}
		val, err := config.GetSetting(p.Key)
		if err != nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings get: %w", err), false, "")
		}
		return &hwcloud.ToolResult{Content: val}
	case "list":
		val, err := config.ListSettings()
		if err != nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings list: %w", err), false, "")
		}
		return &hwcloud.ToolResult{Content: val}
	case "append":
		if strings.TrimSpace(p.Key) == "" {
			return hwcloud.ErrorResult(fmt.Errorf("settings append: key is required"), false, "")
		}
		if err := config.AppendSetting(p.Key, p.Value); err != nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings append: %w", err), true, "")
		}
		return &hwcloud.ToolResult{Content: fmt.Sprintf("appended to %s. Call reload to apply.", p.Key)}
	case "delete":
		if strings.TrimSpace(p.Key) == "" {
			return hwcloud.ErrorResult(fmt.Errorf("settings delete: key is required"), false, "")
		}
		if err := config.DeleteSetting(p.Key); err != nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings delete: %w", err), true, "")
		}
		return &hwcloud.ToolResult{Content: fmt.Sprintf("deleted %s. Call reload to apply.", p.Key)}
	case "validate":
		report, err := config.ValidateSettings()
		if err != nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings validate: %w", err), false, "")
		}
		var msg string
	...
	case "reload":
		...
		if fn == nil {
			return hwcloud.ErrorResult(fmt.Errorf("settings reload: no server running (reload is only available when a server is live)"), false, "")
		}
		result := fn(ctx)
		var parts []string
		...
		return &hwcloud.ToolResult{Content: strings.Join(parts, "\n")}
	case "schema":
		return &hwcloud.ToolResult{Content: settingsSchemaDoc}
	default:
		return hwcloud.ErrorResult(fmt.Errorf("settings: unknown action %q (want set|get|list|append|delete|validate|reload|schema)", action), false, "")
	}
}
...
func newSettingsTool() hwcloud.Tool {
	return &settingsTool{}
}
