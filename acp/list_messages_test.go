...
func (f *fakeMsgStore) Append(context.Context, string, hwcloud.Message) error { return nil }
func (f *fakeMsgStore) Recent(_ context.Context, _ string, n, offset int) ([]hwcloud.Message, error) {
...
func (f *fakeMsgStore) RecentAfter(context.Context, string, int, int) ([]hwcloud.Message, error) {
	return nil, nil
}
...
func TestOnListMessagesMapsAndForwards(t *testing.T) {
	...
	ms := &fakeMsgStore{msgs: []hwcloud.Message{
		{Role: hwcloud.RoleUser, Content: "hello", CreatedAt: &ts},
		{Role: hwcloud.RoleAssistant, Content: "hi", ReasoningContent: "think",
			ToolCalls: []hwcloud.ToolCall{{ID: "tc1", Function: hwcloud.ToolCallFunction{Name: "bash", Arguments: `{"cmd":"ls"}`}}}},
	}}
...
func TestOnListMessagesPagingAndLimitCap(t *testing.T) {
	ms := &fakeMsgStore{msgs: []hwcloud.Message{{Role: hwcloud.RoleUser, Content: "x"}}}
	srv := newListMessagesServer(ms)
