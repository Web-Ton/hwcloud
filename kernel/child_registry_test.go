...
func TestMemSessionStore_AppendAndRecent(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_ = store.Append(ctx, "sub-explorer-1", hwcloud.UserMessage("msg"))
	}
...
func TestMemSessionStore_RecentAfter(t *testing.T) {
	...
	ctx := context.Background()
	for i := 0; i < 10; i++ {
		_ = store.Append(ctx, "s", hwcloud.UserMessage("msg"))
	}
...
func TestMemSessionStore_IsolatedFromParent(t *testing.T) {
	...
	ctx := context.Background()
	_ = childStore.Append(ctx, "sub-explorer-1", hwcloud.UserMessage("child msg"))
...
// stubSummarizer implements hwcloud.Summarizer without calling a model.
type stubSummarizer struct{}

func (stubSummarizer) Summarize(_ context.Context, _ []hwcloud.Message, _ *hwcloud.CompressedContext) (*hwcloud.CompressedContext, error) {
	return &hwcloud.CompressedContext{Summary: "stub summary"}, nil
}
