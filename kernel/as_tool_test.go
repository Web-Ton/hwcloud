...
func (n nameTool) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{Name: string(n)}
}

func (n nameTool) Execute(_ context.Context, _ json.RawMessage) *hwcloud.ToolResult {
...
func TestNew_RegistersSubAgentsAsTools(t *testing.T) {
	...
	rt := New(cfg, Deps{
		Tools: []hwcloud.Tool{nameTool("read")},
	})
...
func TestNew_NoSubAgentsOmitsSendTool(t *testing.T) {
	cfg := agent.New("test")
	rt := New(cfg, Deps{Tools: []hwcloud.Tool{nameTool("read")}})
...
// helper exists because filterChildTools returns []hwcloud.Tool.
func toolNamesFromFilter(tools []hwcloud.Tool) []string {
	out := make([]string, 0, len(tools))
	for _, tl := range tools {
		out = append(out, tl.Definition().Name)
	}
