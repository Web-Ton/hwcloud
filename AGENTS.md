...
- `./hwcloud` at the repo root is a gitignored build artifact; `tmp/` is
...
- Config dir is `~/.<version.Name>/settings.json` (default
  `~/.hwcloud`) — a branded build (`HWCLOUD_BINARY_NAME=myagent`)
  reads `~/.myagent/` instead. `HWCLOUD_CLI_CONFIG` overrides the
...
```sh
./build.sh                # → ./hwcloud (serve/run/keyring + tui subcommand)
```
...
Some verification targets are interactive TUIs (`hwcloud tui`), which
...
```sh
tu run --name oag --size 120x36 --cwd /path ./hwcloud tui   # spawn
